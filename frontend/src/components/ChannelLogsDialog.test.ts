...
describe('ChannelLogsDialog usage', () => {
  it.each([
    ...
    mocks.getChannelLogs.mockResolvedValue({ logs: [{ requestId: 'one', timestamp: '2026-01-01', model: 'test', status: 'completed', interfaceType: 'Responses', usage }] })
    ...
    await flushPromises()
    expect(wrapper.text()).toContain('channelLogs.protocol Responses')
